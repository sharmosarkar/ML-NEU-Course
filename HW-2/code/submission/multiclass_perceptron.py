'''
    ...
    The _activation is sigmoid. In the output layer we use softmax to select the predicted class
'''
...
class Neural_Network(object):
    ...
    def _forward_propagation (self,model, x , is_prediction=False):
        ...
        z1 = x.dot(W1) + b1
        a1 = self._activation ('sigmoid',z1)
        z2 = a1.dot(W2) + b2
        if is_prediction:
            return self._activation('softmax',z2)
        else:
            return a1, self._activation('softmax',z2)

    def predict(self,model, x):
        probs = self._forward_propagation (model, x,True)
        return np.argmax(probs, axis=1)   

    def _activation (self,activation_type, z):
