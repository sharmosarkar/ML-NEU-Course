...
"""
from multi_class_perceptron_support import *
import numpy as np
import pickle
import random
...
OUTPUT_PATH = ""  # Directory in which to save completed models.
...
class MultiClassPerceptron():
    ...
    """
    def __init__(self, classes, feature_data, Y_trn, train_test_ratio=TRAIN_TEST_RATIO, iterations=ITERATIONS):
        ...
        # print(type(self.Y_trn))
        # print(len(self.train_set[0]))

        # Initialize empty weight vectors, with extra BIAS term.
        self.weight_vectors = {c: np.array([0.0 for _ in range(len(self.train_set[0]) + 1)]) for c in self.classes}
    ...
    def train(self):
        ...
        for _ in range(self.iterations):
            for cls, features in zip (self.Y_trn,self.train_set):
                category = str(cls)
                #print(type(features))
                feature_data = features.tolist()
                # Format feature values as a vector, with extra BIAS term.
                #feature_data = [features[k] for k in features]
                feature_data.append(BIAS)
                # print(feature_list)
                # print(len(feature_list))
                feature_vector = np.array(feature_data)
                #print (type(feature_vector), feature_vector)
                ...
                # print(type(predicted_class),predicted_class)
                # print(type(category),category)
                # Update Rule:
                if not (category == predicted_class):
                    self.weight_vectors[str(category)] += feature_vector
...
if __name__ == "__main__":
    ...
    classes = ['0','1','2']
    #print(type(Y_trn[0]),Y_trn[0])
    classifier = MultiClassPerceptron(classes, X_trn, Y_trn)
    classifier.train()
    for features, cls in zip (X_trn, Y_tst):
        print (cls, classifier.predict(features))